...
void exam::info(void)
{
    ...
    for (std::map<int, std::vector<exercise> >::iterator it = lvl_ex.begin(); it != lvl_ex.end(); it++)
    {
        std::cout << "  Level " << LIME << it->first << RESET << ": " << std::endl;
        
        // Display all completed exercises for this level
        for (std::vector<exercise>::iterator ex_it = it->second.begin(); ex_it != it->second.end(); ex_it++)
        {
            for (unsigned int i = 0; i < ex_it->get_assignement(); i++)
            {
                std::cout << "    " << YELLOW << i << RESET << ": " << LIME << ex_it->get_name() << RESET << " for " << (int)level_per_ex_save  << " potential points (" << RED << "Failure" << RESET << ")" << std::endl;
            }
            std::cout << "    " << YELLOW << ex_it->get_assignement() << RESET << ": " << LIME << ex_it->get_name() << RESET << " for " << (int)level_per_ex_save << " potential points (" << LIME << "Success" << RESET << ")" << std::endl;
        }
    }
...
void exam::infovip(void)
{
    ...
    for (std::map<int, std::vector<exercise> >::iterator it = lvl_ex.begin(); it != lvl_ex.end(); it++)
    {
        ...
        for (std::vector<exercise>::iterator ex_it = it->second.begin(); ex_it != it->second.end(); ex_it++)
        {
            for (unsigned int i = 0; i < ex_it->get_assignement(); i++)
            {
                std::cout << "    " << YELLOW << i << RESET << ": " << LIME << ex_it->get_name() << RESET << " for " << (int)level_per_ex_save * (it->first + 1) << " potential points (" << RED << "Failure" << RESET << ")" << std::endl;
            }
            std::cout << "    " << YELLOW << ex_it->get_assignement() << RESET << ": " << LIME << ex_it->get_name() << RESET << " for " << (int)level_per_ex_save * (it->first + 1) << " potential points (" << LIME << "Success" << RESET << ")" << std::endl;
        }
    }
...
void exam::settings_menu(void)
{
    ...
    while (input != "0")
    {
        ...
        std::cout << LIME << "4." << WHITE << BOLD << " Do ALL exercises per level (not just one random)";
        if (setting_all_ex)
            std::cout << LIME << BOLD << " ON" << RESET << std::endl;
        else
            std::cout << RED << BOLD << " OFF" << RESET << std::endl;
