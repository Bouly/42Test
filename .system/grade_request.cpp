...
{
    // insert current_ex in lvl_ex (add to vector for this level)
    int current_level = current_ex->get_lvl();
    if (lvl_ex.find(current_level) == lvl_ex.end())
    {
        // Create new vector for this level
        lvl_ex[current_level] = std::vector<exercise>();
    }
    lvl_ex[current_level].push_back(*current_ex);
    ...
    // Check if we should do all exercises before leveling up
    bool should_level_up = true;
    if (setting_all_ex)
    {
        ...
        if (remaining_list.size() > 0)
        {
            ...
            std::cout << YELLOW << ">>> " << remaining_list.size() << " exercise(s) remaining in level " << level << " <<<" << RESET << std::endl;
            std::cout << "You will get another exercise from the same level." << std::endl << std::endl;
        }
        else
        {
            std::cout << LIME << ">>> All exercises completed for level " << level << "! Moving to next level. <<<" << RESET << std::endl << std::endl;
        }
    }
...
void exam::end_exam()
{
    ...
    // Ask if user wants to clean rendu folder
    std::cout << std::endl
              << "Do you want to clean the 'rendu/' folder? (y/n)" << std::endl;
    char clean_choice;
    std::cin >> clean_choice;
    if (clean_choice == 'y' || clean_choice == 'Y')
    {
        system("rm -rf rendu");
        std::cout << LIME << "✓ rendu/ folder cleaned" << RESET << std::endl;
    }
    else
    {
        std::cout << YELLOW << "→ rendu/ folder kept for review" << RESET << std::endl;
    }
