...
bool exam::clean_all()
{
    // Only delete entire rendu folder if at level 0 (starting fresh)
    // Otherwise, just clean the specific exercise folder
    if (file_exists("rendu") && level == 0)
        system("rm -rf rendu");
    
    // Clean subjects and grading folders (always regenerated)
